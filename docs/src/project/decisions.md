...
**Design:** The core tracks last-activity timestamps for each session and
computes the next wakeup time.
The I/O layer sleeps until that time, then asks the core to check for expired
sessions.
...
class SessionManager:
    def receive_message(
        self, session_id: str, message: dict, now: float,
    ) -> tuple[list[Event], float | None]:
        """Process a message.  Returns (events, next_wakeup_monotonic)."""
        # Updates last-activity for session
        # Returns events + when to next call check_timeouts()
        ...
...
**Rationale:** This follows the sans-IO principle: all timeout logic is pure,
deterministic, and testable without async.
